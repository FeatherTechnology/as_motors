...
$qry = "SELECT 'KRA & KPI' as work_id, kcm.krakpi_calendar_map_id as id, kcm.work_status as sts,DATE(kcm.from_date) as f_date, DATE(kcm.to_date) as t_date,
        ...
        AND 
	( 
        DATE(kcm.from_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
    ) 
AND 
	(
        DATE(kcm.to_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
    )";
...
//Audit start//
$auditTaskInfo ="SELECT 'AUDIT ' as work_id, acr.audit_area_creation_ref_id as id, acr.work_status as sts, ac.audit_area  as title, DATE(acr.from_date) as f_date, DATE(acr.to_date) as t_date FROM audit_area_creation_ref acr LEFT JOIN audit_area_creation ac ON acr.audit_area_id = ac.audit_area_id WHERE ac.status = 0 AND (ac.role1 = '".$designation."' OR ac.role2 = '".$designation."') 
AND 
	( 
        DATE(acr.from_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
    ) 
AND 
	(
        DATE(acr.to_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
    ) ";
...
$assignedTaskInfo = "SELECT 'ASSIGNED WORK ' as work_id, ref_id as id, work_status as sts, work_des_text as title, DATE(from_date) as f_date, DATE(to_date) as t_date FROM assign_work_ref WHERE status = 0 AND designation_id = '".$designation."'
AND 
	( 
        DATE(from_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
    ) 
AND 
	(
        DATE(to_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
    ) "; 
...
//Todo Start //
$todoqry = "SELECT 'TODO ' as work_id, todo_id as id, work_status as sts, work_des as title, DATE(from_date) as f_date, DATE(to_date) as t_date FROM todo_creation WHERE status = 0 AND FIND_IN_SET('$staffid', assign_to) > 0 AND 
( 
    DATE(from_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
) 
AND 
...
WHERE ir.status = 0 
AND ir.designation_id = '".$designation."' 
AND 
( 
    DATE(irr.from_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
) 
AND 
(
    DATE(irr.to_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
)  ";  
...
//FC Ins Start //
$fc_ins_details = $con->query("SELECT 'FC INSURANCE RENEWAL ' as work_id, fc_insurance_renew_id as id, work_status as sts, assign_remark as title, DATE(from_date) as f_date, DATE(to_date) as t_date FROM fc_insurance_renew WHERE status = 0 AND FIND_IN_SET('$staffid', assign_staff_name) > 0  
AND 
( 
    DATE(from_date) BETWEEN DATE('".$staff_from_date."') and DATE('".$staff_to_date."') 
) 
