...
//Get Branch id based on department.
    $getdeptDetails = $con->query("SELECT company_id FROM basic_creation WHERE department = '$dept_name' ");
...
$qry = "SELECT 'KRA & KPI' as work_id, kcm.krakpi_calendar_map_id as id, kcm.work_status as sts,DATE(kcm.from_date) as f_date, DATE(kcm.to_date) as t_date,
        ...
        WHERE kc.status = 0 
        AND kc.department = '".$dept_name."' 
        AND 
	( 
        DATE(kcm.from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
    ) 
AND 
	(
        DATE(kcm.to_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
    )";
...
AND 
( 
    DATE(pcr.from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
) 
AND 
(
    DATE(pcr.to_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
)";
...
AND 
( 
    DATE(bcr.from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
) 
AND 
(
    DATE(bcr.to_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
)";
...
$assignedTaskInfo = "SELECT 'ASSIGNED WORK ' as work_id, ref_id as id, work_status as sts, work_des_text as title, DATE(from_date) as f_date, DATE(to_date) as t_date FROM assign_work_ref WHERE status = 0 AND department_id = '".$dept_name."'
AND 
	( 
        DATE(from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
    ) 
AND 
	(
        DATE(to_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
    ) "; 
...
//Todo Start //
$todoqry = "SELECT 'TODO ' as work_id, todo_id as id, work_status as sts, work_des as title, DATE(from_date) as f_date, DATE(to_date) as t_date FROM todo_creation WHERE status = 0 AND FIND_IN_SET(assign_to, '$staffid') > 0 AND 
( 
    DATE(from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
) 
AND 
...
WHERE ir.status = 0 
AND ir.department_id = '".$dept_name."' 
AND 
( 
    DATE(irr.from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
) 
AND 
(
    DATE(irr.to_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
)  ";  
...
//FC Ins Start //
$fc_ins_details = $con->query("SELECT 'FC INSURANCE RENEWAL ' as work_id, fc_insurance_renew_id as id, work_status as sts, assign_remark as title, DATE(from_date) as f_date, DATE(to_date) as t_date FROM fc_insurance_renew WHERE status = 0 AND FIND_IN_SET(assign_staff_name,'$staffid') > 0  
AND 
( 
    DATE(from_date) BETWEEN DATE('".$dept_from_date."') and DATE('".$dept_to_date."') 
) 
