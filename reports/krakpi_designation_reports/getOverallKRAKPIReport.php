...
$deptStaffList = array();
$deptDesgnList = array();
// get staff based on department id
$getDesignation = "SELECT staff_id, designation FROM staff_creation WHERE department = '".$department_id."' and status = 0 ";
$runQry = $con->query($getDesignation);
while($dept_staff_info = $runQry->fetch_assoc()){
    $deptStaffList[] = $dept_staff_info['staff_id'];
    $deptDesgnList[] = $dept_staff_info['designation'];
}

$staffid = implode(',', $deptStaffList);
$desgnid = implode(',', $deptDesgnList);
...
if($department_id != '0'){
    ...
    $qry = "SELECT 'KRA & KPI' as work_id, DATE(kcr.from_date) as f_date, DATE(kcr.to_date) as t_date, CASE WHEN kcr.rr = 'New' THEN kcr.kpi ELSE rrr.rr END as title, sc.emp_code, sc.staff_name, dc.designation_name   
            ...
            WHERE kc.status = 0 
            AND kc.department = '".$department_id."'
    ...
    while ($krakpitask = $krakpiInfo->fetch()) { 
        ...
        $designationname[]['designationName'] = $krakpitask['designation_name'];
        }
    }
    //KRAKPI END//
    
    //Audit start//
    $auditTaskInfo ="SELECT 'AUDIT ' as work_id, ac.audit_area as title, DATE(acr.from_date) as f_date, DATE(acr.to_date) as t_date, CONCAT(sc1.emp_code,' , ', sc2.emp_code) as emp_code, CONCAT(sc1.staff_name,' , ', sc2.staff_name) as staff_name, CONCAT(dc1.designation_name,' , ', dc2.designation_name) as designation_name 
    ...
    LEFT JOIN designation_creation dc1 ON ac.role1 = dc1.designation_id
    LEFT JOIN designation_creation dc2 ON ac.role2 = dc2.designation_id
    WHERE ac.status = 0 AND ac.department_id = '".$department_id."'
    ...
    while ($audittask = $auditInfo->fetch()) { 
        ...
        $designationname[]['designationName'] = $audittask['designation_name'];
    }   
    } 
    //Audit END//
    
    //Maintance start//          
    $maintanceTaskInfo = "SELECT 'PM MAINTENANCE '  as work_id, pcr.checklist as title, DATE(pcr.from_date) as f_date, DATE(pcr.to_date) as t_date, CONCAT(sc1.emp_code,' , ', sc2.emp_code) as emp_code, CONCAT(sc1.staff_name,' , ', sc2.staff_name) as staff_name, CONCAT(dc1.designation_name,' , ', dc2.designation_name) as designation_name 
    ...
    LEFT JOIN designation_creation dc1 ON mc.role1 = dc1.designation_id
    LEFT JOIN designation_creation dc2 ON mc.role2 = dc2.designation_id
    WHERE mc.status = 0 AND ( FIND_IN_SET('$desgnid', mc.role1) > 0 OR FIND_IN_SET('$desgnid', mc.role2) > 0 )
    ...
    //BM Start//
    $bmTaskInfo = "SELECT 'BM MAINTENANCE ' as work_id, bcr.checklist as title, DATE(bcr.from_date) as f_date, DATE(bcr.to_date) as t_date, CONCAT(sc1.emp_code,' , ', sc2.emp_code) as emp_code, CONCAT(sc1.staff_name,' , ', sc2.staff_name) as staff_name, CONCAT(dc1.designation_name,' , ', dc2.designation_name) as designation_name 
    ...
    while ($bmtask = $bmInfo->fetch()) { 
        ...
        $designationname[]['designationName'] = $bmtask['designation_name'];
    }
    }
    //BM END//
    ...
    ?>      
        <tr>
            ...
            <td><?php echo $designationname[$i]['designationName']; ?></td>
...
if($department_id != '0'){
    ...
    while ($maintancetask = $maintanceInfo->fetch()) { 
        ...
        $empcode[]['empCode'] = $maintancetask['emp_code'];
        $staffname[]['staffName'] = $maintancetask['staff_name'];
        $designationname[]['designationName'] = $maintancetask['designation_name'];
    }
    } 
    //Maintance END//
    ...
    //campaign Start//
    $campaignTaskInfo = "SELECT 'CAMPAIGN ' as work_id, cf.activity_involved as title, DATE(cf.start_date) as f_date, DATE(cf.end_date) as t_date, sc.emp_code, sc.staff_name, dc.designation_name 
    ...
    WHERE c.status = 0 AND FIND_IN_SET('$department_id', department_id) > 0 
    AND 
    ( 
    ...
    while($campaigntask = $campaignInfo->fetch_assoc()) {
        ...
        $designationname[]['designationName'] = $campaigntask['designation_name'];
    }
    }
    //campaign END//
    
    //assign work list Start//
    $assignedTaskInfo = "SELECT 'ASSIGNED WORK ' as work_id, ref_id as id, work_status as sts, work_des_text as title, DATE(from_date) as f_date, DATE(to_date) as t_date, sc.emp_code, sc.staff_name, dc.designation_name 
    FROM assign_work_ref awf 
    LEFT JOIN staff_creation sc ON awf.designation_id = sc.designation AND sc.status = 0 
    LEFT JOIN designation_creation dc ON awf.designation_id = dc.designation_id
    WHERE awf.status = 0 AND awf.department_id = '".$department_id."'
    AND 
        ( 
    ...
    while($assignTask = $assignInfo->fetch_assoc()) { 
        ...
        $designationname[]['designationName'] = $assignTask['designation_name'];
    }
    }
    //assign work list END//
    
    //Todo Start //
    $todoqry = "SELECT 'TODO ' as work_id, work_des as title, DATE(from_date) as f_date, DATE(to_date) as t_date, sc.emp_code, sc.staff_name, dc.designation_name 
    ...
    while($todoinfo = $gettodoinfo->fetch_assoc()){
        ...
        $designationname[]['designationName'] = $todoinfo['designation_name'];
    }
    }
    //ToDo END //
    
    //Insurance Register Start //
    $insqry = "SELECT 'INSURANCE REGISTER ' as work_id, DATE(irr.from_date) as f_date, DATE(irr.to_date) as t_date,ir.insurance_id as ins_id, sc.emp_code, sc.staff_name, dc.designation_name 
    ...
    LEFT JOIN designation_creation dc ON ir.designation_id = dc.designation_id 
    WHERE ir.status = 0 
    ...
    while($insInfo = $insdeatils->fetch_assoc()){
        ...
        $designationname[]['designationName'] = $insInfo['designation_name'];
    }
    }
    //Insurance Register END //
    
    //FC Ins Start //
    $fc_ins_details = $con->query("SELECT 'FC INSURANCE RENEWAL ' as work_id, fc_insurance_renew_id as id, work_status as sts, assign_remark as title, DATE(from_date) as f_date, DATE(to_date) as t_date, sc.emp_code, sc.staff_name, dc.designation_name  
    ...
    while($fcInfo = $fc_ins_details->fetch_assoc()){
        ...
        $designationname[]['designationName'] = $fcInfo['designation_name'];
    }
    //FC Ins END //
