...
$vehicle_details = $mysqli->query("SELECT cc.company_name, bc.branch_name, sc.staff_name, vd.vehicle_number as vnum, ds.* FROM `diesel_slip` ds LEFT JOIN vehicle_details vd ON ds.vehicle_number = vd.vehicle_details_id LEFT JOIN branch_creation bc ON ds.company_id = bc.branch_id LEFT JOIN company_creation cc ON bc.company_id = cc.company_id LEFT JOIN staff_creation sc ON ds.staff_id = sc.staff_id order by ds.diesel_slip_id DESC");
$sno = 1;
