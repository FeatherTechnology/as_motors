    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(userid != '1'){
                ...
                data.addRows([
                    [{'v':'Organization Chart', 'f':'<label style="width:200px; font-size:20px; text-align:center;">Organization Chart</label>'},'', 'Organization Chart']]);

                data.addRows([
                    [{'v':response['company_name'][0], 'f':'<div style="width:200px; align-content:center;"><b>Company Name: </b>'+ response['company_name'][0] +'<br><b>Branch Name:</b>'+ response['branch_name'][0]+'</div'},'Organization Chart', 'Company Details']]);
                    
                data.addRows([
                    [{'v':response.userbranch_id, 'f':'<div style="width:50px;text-align:center;">'+response.userfullname+'</div>'},'Organization Chart', 'Director']]);
                ...
                //         response.userbranch_id, 'Company']
                //     ]);
                ...
                //             data.addRows([
                //                 [{'v':response['branch_id'][ibranch], 'f':response['branch_name'][ibranch]+ ' Branch'}, response['company_name'][i],'Branch']
                //             ]);
                            ...
                            for(var idep = 0; idep < response.hierarchy_id.length; idep++){
                                ...
                                // if(response['hierarchy_branch_id'][idep] == response['branch_id'][ibranch]){
                                    data.addRows([
                                        [{'v':response['department_id'][idep]+'dep', 'f':response['department_name'][idep]}, response.userbranch_id,'Department']
                                    ]);
                ...
                // }
                data.addRows([
                    [{'v':response['branch_address'][0], 'f':'<div style="width:200px;text-align:center;"><b>Address:</b><br>' + response['branch_address'][0] +'</div'},'Organization Chart', 'Address']]);
                
            }else{
                // For each orgchart box, provide the name, manager, and tooltip to show.
                data.addRows([
                    [{'v':response.userbranch_id, 'f':response.userfullname},'', 'Director']]);
                    ...
                    for(var ibranch = 0; ibranch < response.branch_id.length ; ibranch++){
                        if(response['branch_company_id'][ibranch] == response['company_id'][i]){
                            ...
                            for(var idep = 0; idep < response.hierarchy_id.length; idep++){
                                ...
                                if(response['hierarchy_branch_id'][idep] == response['branch_id'][ibranch]){
                                    data.addRows([
                                        [{'v':response['department_id'][idep]+'dep', 'f':response['department_name'][idep]}, response['branch_id'][ibranch],'Department']
                                    ]);
