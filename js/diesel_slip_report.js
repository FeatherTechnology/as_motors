$(document).ready(function(){ //Document Ready Start.
    ...
    $('#diesel_slip_report_type').change(function(){ //Onchange based on type select for report.
    ...
    });//diesel_slip_report_type END.

    $('#view_report').click(function(){ //view report in the table.
        var reportType = $('#diesel_slip_report_type').val();
...
function getOverAllReport(){
    ...
    $.ajax({
        type: "POST",
        data: {},
        url: "reports/diesel_slip_reports/getDieselSlipOverallReport.php",
        cache: false,
        success: function(response){

            $("#dieselSlip_report_view_table").empty();
            $("#dieselSlip_report_view_table").html(response);
        }
    })//Report ajax END.
}
...
function branchBasedReports(branch_id, from_date, to_date){
    ...
    $.ajax({
        ...
        url: "reports/diesel_slip_reports/getDieselSlipBranchBasedReport.php",
        cache: false,
        success: function(response){
...
function tableEmpty(){
    $("#dieselSlip_report_view_table").empty(); //if validation false then table will empty.
}
