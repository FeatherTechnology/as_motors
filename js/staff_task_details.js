$(document).ready(function(){ //Document Ready Start.
    ...
    $('#task_details_type').change(function(){ //Onchange based on type select for report.
        var typeValue = $(this).val();

        if(typeValue == '2'){
            $('#department_task').hide();
            $('#designation_task').show();
            getDepartmentList('dept_name') //Get All Department list.
            
        } else{
            $('#department_task').show();
            $('#designation_task').hide();
    ...
    });//task_details_type END.

    $('#view_task').click(function(){ //view report in the table.
        var reportType = $('#task_details_type').val();
...
$(function(){
    ...
    if(role == 4){
        $('#task_details_type').val('2');
        $('#task_details_type').attr('disabled', true);
        $('#department_task').hide();
        $('#designation_task').show();
        getDepartmentList('dept_name') //Get All Department list.
        $('#dept_name').attr('disabled', true);
...
function getOverAllReport(department_id){
    ...
    $.ajax({
        ...
        url: "reports/staff_task_details/getOverallTaskDetails.php",
        cache: false,
        success: function(response){

            $("#task_details_view_table").empty();
            $("#task_details_view_table").html(response);
        }
    })//Report ajax END.
}
...
function designationBasedReports(designation_id){
    ...
    $.ajax({
        ...
        url: "reports/staff_task_details/designationBasedTaskDetails.php",
        cache: false,
        success: function(response){
...
function tableEmpty(){
    $("#task_details_view_table").empty(); //if validation false then table will empty.
}
