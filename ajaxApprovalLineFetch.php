...
foreach ($result as $row) {
    ...
    $approval_line_id='';
    $qry50 = "SELECT * FROM approval_requisition WHERE approval_line_id = '".$id."' AND status=0"; 
    $res50 = $con->query($qry50);
    while($row50 = $res50->fetch_assoc())
    {
        $approval_line_id = $row50['approval_line_id'];
    }
	
    if($approval_line_id > 0){
        $action="<a href='approval_requisition&dashupd=$id' title='Edit details'><button class='btn btn-info printpo'>View</button></a>";
    }else {
        $action="<a href='approval_requisition&upd=$id' title='Edit details'><button class='btn btn-info printpo'>Proceed</button></a>";
    }
