...
$query = "SELECT * FROM tag_creation WHERE 1";
    ...
    if($_POST['search']!="");
    {
        if (isset($_POST['search'])) {
            ...
            else if($_POST['search']=="Inactive")
            {
                ...
                OR tag_classification LIKE  '%".$_POST['search']."%'
...
foreach ($result as $row) {
    ...
    $branch_name = array();
    $company_name = array();
    $companyIdArr = explode(",", $row['company_id']);
    foreach ($companyIdArr as $companyId) {

        $qry = "SELECT * FROM branch_creation WHERE branch_id = '".strip_tags($companyId)."' AND status=0 ORDER BY branch_id DESC"; 
        $res = $con->query($qry);
    ...
    $sub_array[] = $row['tag_classification'];
	...
	$action="<a href='tag_creation&upd=$id' title='Edit details'><span class='icon-border_color'></span></a>&nbsp;&nbsp; 
	<a href='tag_creation&del=$id' title='Delete details' class='delete_tag_creation'><span class='icon-trash-2'></span></a>";
...
{
    $query     = "SELECT * FROM tag_creation";
    $statement = $connect->prepare($query);
    $statement->execute();
