...
{ 
    $idupd=$_GET['upd'];
    $addBusinessComOutApproveStaff = $userObj->getBusinessComOutApproveStaff($mysqli, $idupd); 
    if (sizeof($addBusinessComOutApproveStaff)>0) {   
        for($a=0;$a<sizeof($addBusinessComOutApproveStaff);$a++) {	
            $approval_line_id                    = $addBusinessComOutApproveStaff['business_com_line_id'];
            $company_id                	         = $addBusinessComOutApproveStaff['company_id'];
            $staff_id                	         = $addBusinessComOutApproveStaff['staff_id'];
            $approval_staff_id                   = $addBusinessComOutApproveStaff['approval_staff_id'];
            $agree_par_staff_id		             = $addBusinessComOutApproveStaff['agree_par_staff_id'];
            $after_notified_staff_id		     = $addBusinessComOutApproveStaff['after_notified_staff_id'];
            $receiving_dept_id		             = $addBusinessComOutApproveStaff['recipient_id'];
        }
        // approval staff
...
{   
    ...
    if(isset($_GET['receivingBranch'])){
        $receivingBranch=$_GET['receivingBranch'];
    } 

    $dashupd=$_GET['dashupd'];
    $addBusinessComOutApproveStaff = $userObj->getBusinessComOutApproveStaff($mysqli, $dashupd); 
    if (sizeof($addBusinessComOutApproveStaff)>0) {   
        for($a=0;$a<sizeof($addBusinessComOutApproveStaff);$a++) {	
            $approval_line_id                    = $addBusinessComOutApproveStaff['business_com_line_id'];
            $company_id                	         = $addBusinessComOutApproveStaff['company_id'];
            ...
            $after_notified_staff_id		     = $addBusinessComOutApproveStaff['after_notified_staff_id'];
            $receiving_dept_id		             = $addBusinessComOutApproveStaff['recipient_id'];
            $drafter_approval_date		         = $addBusinessComOutApproveStaff['created_date'];
            $checker_approval_date		         = $addBusinessComOutApproveStaff['checker_approval_date'];
            $reviewer_approval_date		         = $addBusinessComOutApproveStaff['reviewer_approval_date'];
            $final_approval_date		         = $addBusinessComOutApproveStaff['final_approval_date'];
            $checker_approval		             = $addBusinessComOutApproveStaff['checker_approval'];
            $reviewer_approval		             = $addBusinessComOutApproveStaff['reviewer_approval'];
            $final_approval		                 = $addBusinessComOutApproveStaff['final_approval'];   
        } 
...
$id=0;
if(isset($_POST['businessCommunicationSubmit']) && $_POST['businessCommunicationSubmit'] != '')
{
    ...
    <?php }
        ...
        $addBusinessComOut = $userObj->addBusinessComOut($mysqli,$userid);   
        ?>
        <script>location.href='<?php echo $HOSTPATH; ?>business_com_out';</script>
        <?php
    }
}   
...
{
    if($parallel > 0){
        $parallelAgreeBusinessComOut = $userObj->parallelAgreeBusinessComOut($mysqli, $userid);   
    }else{
        $agreeBusinessComOut = $userObj->agreeBusinessComOut($mysqli, $userid);   
    }
    ?>
...
{
    if($parallel > 0){
        $parallelDisagreeBusinessComOut = $userObj->parallelDisagreeBusinessComOut($mysqli, $userid);     
    }else{
        $disagreeBusinessComOut = $userObj->disagreeBusinessComOut($mysqli, $userid);    
    }
    ?>
    <script>location.href='<?php echo $HOSTPATH; ?>dashboard';</script>
    <?php
...
<?php if($dashupd > 0){ ?>
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6">
                            ...
                            <br>   
                            <?php 
                            if($afterNotification <= 0 && $receivingBranch <= 0){ ?> 
                                <div class="text-left" >
    ...
    <div style="display: none;" id="printBusinessComOutDiv"></div>
                ...
                { ?>
                    <div class="col-md-12">
                        ...
                        <div class="text-right">
                            ...
                            <button type="submit" name="businessCommunicationSubmit" id="businessCommunicationSubmit" class="btn btn-primary" value="Submit" tabindex="4">Submit</button>
