...
foreach ($result as $row) {
    ...
    $business_com_line_id='';
    $qry50 = "SELECT * FROM business_com_out WHERE business_com_line_id = '".$id."' AND status=0"; 
    $res50 = $con->query($qry50);
    while($row50 = $res50->fetch_assoc())
    {
        $business_com_line_id = $row50['business_com_line_id'];
    }
	
    if($business_com_line_id > 0){
        $action="<a href='business_com_out&dashupd=$id' title='Edit details'><button class='btn btn-info printpo'>View</button></a>";
    }else {
        $action="<a href='business_com_out&upd=$id' title='Edit details'><button class='btn btn-info printpo'>Proceed</button></a>";
    }
