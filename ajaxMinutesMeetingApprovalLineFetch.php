...
foreach ($result as $row) {
    ...
    $meeting_minutes_approval_line_id='';
    $qry50 = "SELECT * FROM meeting_minutes WHERE meeting_minutes_approval_line_id = '".$id."' AND status=0"; 
    $res50 = $con->query($qry50);
    while($row50 = $res50->fetch_assoc())
    {
        $meeting_minutes_approval_line_id = $row50['meeting_minutes_approval_line_id'];
    }
	
    if($meeting_minutes_approval_line_id > 0){
        $action="<a href='meeting_minutes&dashupd=$id' title='Edit details'><button class='btn btn-info printpo'>View</button></a>";
    }else {
        $action="<a href='meeting_minutes&upd=$id' title='Edit details'><button class='btn btn-info printpo'>Proceed</button></a>";
    }
