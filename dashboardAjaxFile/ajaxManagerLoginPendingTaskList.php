...
$campgnTaskInfo = "SELECT 'CAMPAIGN' as tb, activity_involved as title, end_date as end_date, employee_name as assign, work_status as sts, campaign_ref_id FROM campaign_ref WHERE work_status != 3 AND ( `end_date` >= '$curdate' AND `end_date` <= '$curdate' + INTERVAL 10 DAY ) AND department_id = '$userDept' ";
...
$fcinsTaskInfo = "SELECT 'FC INSURANCE RENEW' as tb, assign_remark as title, work_status as sts, to_date as end_date, assign_staff_name as assign, fc_insurance_renew_id FROM `fc_insurance_renew` WHERE work_status != 3 AND (`to_date` >= '$curdate' AND `to_date` <= '$curdate' + INTERVAL 10 DAY ) AND FIND_IN_SET('$deptstaffid', assign_staff_name)";
...
$assignworkTaskInfo = "SELECT 'ASSIGN WORK' as tb, work_des_text as title, to_date as end_date, designation_id as assign, work_status as sts, ref_id FROM assign_work_ref WHERE work_status != 3 AND status = 0 AND ( `to_date` >= '$curdate' AND `to_date` <= '$curdate' + INTERVAL 10 DAY) AND FIND_IN_SET('$userDept', department_id)";
