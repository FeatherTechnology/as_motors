...
'" . strip_tags($work_status) . "',
'" . strip_tags($remarks) . "', current_timestamp())  ";
...
if (strstr($workdes_id, $ifhas)) {
    ...
    $qry = "UPDATE todo_creation set work_status = 2 where todo_id = '".$todo_id."' ";
    ...
    $qry = "UPDATE krakpi_calendar_map set work_status = 2 where krakpi_calendar_map_id = '".$krakpi_calendar_map_id."' ";
    ...
    $qry = "UPDATE audit_area_creation_ref set work_status = 2 where audit_area_creation_ref_id = '".$audit_area_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
} 
else if(strstr($workdes_id, $ifhas3)) {
    //"The substring was found in the string";
    $maintenance_checklist_id = preg_replace('/maintenance /', '', $workdes_id);
    $qry = "UPDATE pm_checklist_ref set work_status = 2 where pm_checklist_ref_id = '".$maintenance_checklist_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
} else if(strstr($workdes_id, $ifhas4)) {
    //"The substring was found in the string";
    $campaign_ref_id = preg_replace('/campaign /', '', $workdes_id);
    $qry = "UPDATE campaign_ref set work_status = 2 where campaign_ref_id = '".$campaign_ref_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
} else if(strstr($workdes_id, $ifhas5)) {
    //"The substring was found in the string";
    $ins_reg_ref_id = preg_replace('/insurance /', '', $workdes_id);
    $qry = "UPDATE insurance_register_ref set work_status = 2 where ins_reg_ref_id = '".$ins_reg_ref_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
} else if(strstr($workdes_id, $ifhas6)) {
    //"The substring was found in the string";
    $maintenance_checklist_id_bm = preg_replace('/BM /', '', $workdes_id);
    $qry = "UPDATE bm_checklist_ref set work_status = 2 where bm_checklist_ref_id = '".$maintenance_checklist_id_bm."' ";
    ...
    $qry = "UPDATE fc_insurance_renew set work_status = 2 where Fc_insurance_renew_id = '".$Fc_insurance_renew_id."' ";
    ...
    $qry = "UPDATE assign_work_ref set work_status = 2 where ref_id = '".$assign_work_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
}
