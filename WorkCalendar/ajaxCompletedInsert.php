...
if (strstr($workdes_id, $ifhas)) {
    ...
    $qry = "UPDATE krakpi_creation_ref set work_status = 3 where krakpi_ref_id = '".$krakpi_ref_id."' ";
    ...
    $qry = "UPDATE audit_area_creation set work_status = 3 where audit_area_id = '".$audit_area_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
} else if(strstr($workdes_id, $ifhas3)) {
    //"The substring was found in the string";
    $maintenance_checklist_id = preg_replace('/maintenance /', '', $workdes_id);
    $qry = "UPDATE maintenance_checklist set work_status = 3 where maintenance_checklist_id = '".$maintenance_checklist_id."' ";
    $result = $con->query($qry) or die("Error Not able to update assign work table");
} 
